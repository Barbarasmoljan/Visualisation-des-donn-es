...
Une des limites de ce type de visualisation concerne la difficulté de pouvoir effectuer un comparaison de manière précise pour des valeurs qui seraient proches, telles qu'une valeur représentant 22 % par rapport à une valeur représentant 25 %. Contrairement à des visualisations ou des graphiques qui représentent des barres ou des histogrammes, il est plus difficle de lire et comparer les tranches/ zones issues de ce type de visualisation. De plus, sans texte ou explication, il serait beaucoup plus difficle de la comprendre. Ce type visualisation n'est pas non plus adaptée s'il y avait eu des données beaucoup plus nombreuses. Un autre aspect limitant porte sur le fait qu'il s'agit d'un vinyle qui tourne. Ceci peut désorienter les utilisateur.ice.s et donc leur faire predre leurs reprères par rapport aux tranches d'âges/ zones colorées.

## Crédits

## 5) Piano

Cette nouvelle visualisation représente des réponse à une question qui traite des diffuclté rencontré lors de l'apprentissage musical et use de la métaphore visuel d'un piano. Le piano  comme instrument est souvent utilisé comme un symbole  en général de l'apprentissage de la musique. L'idée de cette visualisation consitait d'apporter un effet ludique et interactif de l'usage des données. La structure de la visualisation s'appuie sur la binarité des réponse "Oui" ou "NON", pour ensuitre illustre la proportion des personnes ayant recontré ou non des difficultés dans leur apprentisage.

Un des objetifs de cette visualisation est d'offir la possiibilité à un.e utilisateur.ice  d'explorer les deux résultats possible en jourant différentes touche pour découvrir les réponses. Pour ce faire, il est possible de tester ces résultats en cliquant au hasard sur des touches du piano pour tenter de retrouver les deux notes associées aux réponse " OUI" et "NON" ou directementen appuyant sur les touches "V" et G"  du clavier d'ordinateur .  La raison du choix de ces deux lettres pour rappeler le positionnement des doigts sur des touches de clavier. La visualisation affiche de façon claire, si la réponse correspon à "OUI" ou "NON". Elle affiche également le pourcentage et le nombre précis de personnes ayant répondu dans chacun des cas.

L'idée de la visualisation est de permettre aux utilisateu.ice.s de pouvoir intéragir soit en cliquant au hasard sur des touches du piano soit en utilisant les touches "V" ou "G" d'un clavier d'ordinateur. En procédant ainsi, ceci déclenche la lecture d'une note (son d'une note spécifique). Lorsqu'une toouche spécifique est activées, celle-ci s'anime. En effet, il est possible d'observer que la touche bouge (descend, puis remonte) afin de simuler une pression d'un vrai clavier de piano. Le qui accompagne cela renfornce l'aspect mutlisensoriel de l'expérience. Le fonctionnement de cette visualisation se veut être simple et chaque interaction génère un feedback immédiat afin que les personnes puisse comprendre la répartitions des réponses. 

Une autre volonté était d'obtenir un effet sensoriel et sonore tout en proposant un exploration ludique.

Une des limites de cette visualiation concernen la perception des proporitions qui peut être diffficle à interpréter de façon précise. Sans explication écrite il serait compliqué de comprendre les valeur et leurs significations. Une autre limite réside dans le fait que cette visualisation pourrait être confondue à une pure simulation musical et que chaque touches pourrait représenter d'autres données. 
Un autre aspect important  à souligner et qui n'est pas explicitement spécifié dans cette visualisation concerne le nombre de réponse "OUI" et "NON". Nous obtenons 97 réponses sur un total de 111 pour l'entier du sondage. Cependant, si on additionne le nombre de "OUI" et le nombre de "NON", on n'arrive pas à 111. Ceci suggère que toutes les réponses ne sont pas comptabilisées ou disponibles. Il est possible d'expliquer ceci notamment par le format du sondage et dans lequel la question traitée n'était pas formulée comme un question obligatoire. Ce qui fait qu'une partie des personnes ayant pris part au sondage ont dû laissé des cette question vide ou sont passé par-dessus. Par conséquent, la répartition des "OUI" et des "NON" ne couvre pas la totalité des personnes qui ont répondu au sondage. 
