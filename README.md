...
Pour simuler l’aspect d’un véritable vinyle, une aiguille (needle dans le code) composée d’un rectangle et de deux petits cercles à son extrémité a été ajouté. Cette aiguille s’active lors du clic sur une zone colorée du vinyle. Enfin, en arrière-plan, des notes de musiques animées ont été intégrées. Celles-ci montent de façon fluide pour renforcer l’ambiance musicale de la visualisation. 

Une des limites de ce type de visualisation concerne la difficulté de pouvoir effectuer un comparaison de manière précise pour des valeurs qui seraient proches, telles qu'une valeur représentant 10.8 % par rapport à une valeur représentant 8.1 %. Contrairement à des visualisations ou des graphiques qui représentent des barres ou des histogrammes, il est plus difficile de lire et de comparer les tranches/ zones issues de ce type de visualisation. De plus, sans texte ou explication, il serait beaucoup plus complexe de la comprendre. Ce type visualisation ne serait pas non plus adaptée s'il y avait des données beaucoup plus nombreuses. Un autre aspect limitant porte sur le fait qu'il s'agit d'un vinyle qui tourne. Ceci peut désorienter les utilisateur.ice.s et donc leur faire perdre leurs reprères par rapport aux tranches d'âges/ zones colorées.
...
Un des objetifs est d'offir la possiibilité à un.e utilisateur.ice d'explorer les deux résultats possibles en jouant différentes touches pour découvrir les réponses. Pour ce faire, il est possible de tester ces résultats en cliquant au hasard sur des touches du piano pour tenter de retrouver les deux notes associées aux réponse " OUI" et "NON" ou directement en appuyant sur les touches "V" et G" du clavier d'ordinateur. La raison du choix de ces deux lettres est pour rappeler le positionnement des doigts sur des touches d'un clavier de piano. La visualisation affiche de façon claire, si la réponse correspond à "OUI" ou "NON". Elle affiche également le pourcentage et le nombre précis de personnes ayant répondu dans chacun des cas.

L'idée de la visualisation est de permettre aux utilisateu.ice.s de pouvoir intéragir soit en cliquant au hasard sur des touches du piano soit en utilisant les touches "V" ou "G" d'un clavier d'ordinateur. En procédant ainsi, ceci déclenche la lecture d'une note (son d'une note spécifique). Lorsqu'une touche spécifique est activée, celle-ci s'anime. Il est également possible d'observer que la touche bouge (descend, puis remonte) afin de simuler une pression sur une touche de piano. Le son qui accompagne l'action renforce l'aspect mutlisensoriel de l'expérience. Le fonctionnement de cette visualisation se veut être simple et chaque interaction génère un feedback immédiat afin que les personnes puisse comprendre la répartitions des réponses. Le but étant d'obtenir un effet sensoriel et sonore, tout en proposant une petite exploration ludique.

Une des limites de cette visualiation concerne la perception des proportions qui peut être difficile à interpréter de façon précise. Sans explication écrite, il serait compliqué de comprendre les valeurs et leurs significations. Une autre limite réside dans le fait que cette visualisation pourrait être confondue à une pure simulation musicale et que chaque touche pourrait représenter d'autres données. 

Un autre aspect important à souligner et qui n'est pas explicitement spécifié dans cette visualisation concerne le nombre de réponse "OUI" et "NON". Nous obtenons 97 réponses sur un total de 111 pour l'entier du sondage. Cependant, si on additionne le nombre de "OUI" et le nombre de "NON", on n'arrive pas à 111. Ceci suggère que toutes les réponses ne sont pas comptabilisées ou disponibles. Ceci s'explique notamment par le format du sondage dans lequel la question traitée n'était pas formulée comme une question obligatoire. Ce qui fait qu'une partie des personnes ayant pris part au sondage ont dû laisser cette question vide ou sont passées par-dessus. Par conséquent, la répartition des "OUI" et des "NON" ne couvre pas la totalité des personnes qui ont répondu au sondage. 

Malgré ces limites , il est possible de dire que cette visualisation propose une manière différente d'explorer et interroger des données. Elle s'émancipe d'une présentation graphique habituelle pour créer une expérience plus interactive et mulisensorielle.

## Crédits

Sons pour les notes proviennent de site SampleFocus : https://samplefocus.com/
